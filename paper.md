...
This goes to show not only that Lean is at least as expressive as ZFC, but also the consistency of ZFC relative to Lean.
...
In this paper we implemented first-order logic as an internal embedded language of Lean and used Lean structures to give model-theoretic semantics to the logical calculus. This allowed us to derive the consistency of the calculus, and opens up the way for future work in this area.
...
As can be seen, we use the {$\forall, \to,\bot$} functionally complete fragment of first-order logic in the definition of formulas. For the `if_then` constructor we set up a specific notation, since Lean will allow us to do that:
...
Rewriting is used in the definition (`φ.rw`) as well as the notion of a variable being substitutable by a term in a formula. We also require defining the notion of a variable being free in a formula, and the notion of a variable being "abstract" in a set of formulas. We will detail the implementation of these in the following sections.
...
Given these definitions, all dependencies of our deductive system are defined. 

<!-- We can then provide some simple examples of proofs. -->

<!-- ### **Proof examples**

First we declare sets of formulas and formulas to be referenced in the context of our proofs:
...
Essentially here we apply the $\to-\text{introduction}$ rule to prove $\varphi \to \varphi$. The goal then becomes to prove $\Gamma \cup \{\varphi\} \vdash \varphi$. We can prove this by noticing that $\varphi \in \Gamma \cup \{\varphi\}$, therefore by reflexivity $\Gamma \cup \{\varphi\} \vdash \varphi$, and the proof is done.

Then we provide an example of how to prove a simple syntatical meta-theorem:
...
It is then trivial to define a structure for the language, which we call a model:
...
We have chosen to follow the pattern that the functions defined with a `'` depend on a choice of assignment, while the ones without it do not. Finally, we define the notion of a formula being a semantic consequence of a set of formulas:
...
The proof works by constructing some model $M$, which is trivial to do, and then this will be a model of the empty set. It follows by soundness that if `formula.false` could be proven from $\empty$, then $M$ would satisfy `formula.false`, but since no model can do that, we have that $\empty$ does not prove `formula.false`.
...
- Talk about a future internalized (deep embbeding) implementation of Hoare logic and how it could be used for program verification. -->
...
[2] @article{The_mathlib_Community_2020,
...
[3] Curry, Haskell B. and Robert Feys, 1958, Combinatory Logic, Amsterdam: North-Holland.  
[4] 2017, Jeremy Avigad, Robert Y. Lewis, and Floris van Doorn. Logic and Proof. https://leanprover.github.io/logic_and_proof/ (accessed in 4/29/2020).  
[5] @misc{han2019formalization,
...
[7] A Mathematical Introduction to Logic. Herbert B. Enderton
<!-- [8] http://wwwf.imperial.ac.uk/~buzzard/xena/natural_number_game/ (accessed in 4/29/2020). -->